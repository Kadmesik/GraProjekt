    ...
    public override bool ProcessKeyboard(SadConsole.Input.Keyboard keyboard)
        {
            ...
            if (keyboard.IsKeyPressed(SadConsole.Input.Keys.Right))
            {
                if (Slider < 9)
                {
                    ...
                    Slider++;
                    thiefbility++;
                    DrawingTools.DrawAvatar(_mainSurface, selectedColor[thiefbility]);
                }
            }
            ...
            if (keyboard.IsKeyPressed(SadConsole.Input.Keys.Enter))
            {
                ...
                _mainSurface.Print(30, 22, $"Zatwierdzono! Karnacja to: {CarnationName[thiefbility]}");

                PlayerStats player = new PlayerStats
                {
                Carnation = CarnationName[thiefbility],
                };
